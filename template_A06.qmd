...
```{r}
mat_rand <- apply(mat_rand, MARGIN=1:2, function(x) signif(x, 1)) #rounds to 1 significant digit
...
for(i in 3:ncol(max_temps)){
  mean_temps <- append(mean_temps, mean(unlist(max_temps[,i])))
  #calculates the mean of each month's column (unlisted to obtain numeric datatypes) and appends to a new vector
}
