...
[https://github.com/emmavnu/Vinokour_Emma_201_A06.git](https://github.com/emmavnu/Vinokour_Emma_201_A06.git)
...
```{r}
player_heights <- list(MJ = 78, LJ = 81, KB = 78, KA = 86, SO = 85)
...
#iterates through each player's info list
for (i in 1:length(player_info)){ 
  #calculates and adds each player's points per game to the new list
  points_per_game <- append(points_per_game, player_info[[i]][[4]]/player_info[[i]][[3]])
}
...
```{r}
points_vec <- sapply(points_per_game, function(x) round(x,2))
...
```{r}
special_round <- function(num){
  if (num%/%10 >=1 && num%/%10 <=9 ) { #uses floor division to check if number is 2 digits (number of 10s in the number must be between 1 & 9)
    return(round(num, -1)) #rounds to the nearest 10
  } else if (num%/%100 >=1 && num%/%100 <=9) { #uses floor division to check if number is 3 digits (number of 100s in the number must be between 1 & 9)
    return(round(num, -2)) #rounds to the nearest 100
  } else if (num%/%1000 >=1 && num%/%1000 <=9) { #uses floor division to check if number is 4 digits (number of 1000s in the number must be between 1 & 9)
    return(round(num, -3)) #rounds to the nearest 1000
  }
}

mat_rand <- apply(mat_rand, MARGIN=1:2, special_round)
...
for(i in 3:ncol(max_temps)){
  mean_temps <- append(mean_temps, mean(max_temps[,i])) 
  #calculates the mean of each month's column and appends to a new vector
}

names(mean_temps) <- c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")
...
```{r}
median_temps_co <- list()

for(i in 1:nrow(max_temps)){
  median_temps_co <- append(median_temps_co, median(as.numeric(co_temps[i,3:14])))
  #iterates through each applicable row in data frame, slices it to exclude the state name and year, uses the unlist() function to convert type from a list to a numeric vector, then uses the median function on the vector and appends the median value to the median_temps_co list
}

max_index <- which.max(median_temps_co)
min_index <- which.min(median_temps_co)

print(paste0("The maximum median annual temperature of ", median_temps_co[[max_index]], " occurred in ", co_temps[max_index, 2], " and the minimum median annual temperature of ", median_temps_co[[min_index]], " occurred in ", co_temps[min_index, 2], "."))
...
digit_sum <- function(num){
  ...
  if(num==0){
    ...
    #finds the last digit of number by finding remainder when divided by 10, then continually adds to the truncated number, of which it also finds the last digit, etc.
